 ...
 */
export async function ensurePointsRecord(employeeId) {
 ...
 */
export async function checkAndResetMonthlyPoints(pointsRecord) {
 ...
 */
export async function addAttendancePoints(employeeId, status, isOnTime = true) {
 ...
 */
export async function addDailyUpdatePoints(employeeId, updateType) {
    const pointsRecord = await ensurePointsRecord(employeeId);
    await checkAndResetMonthlyPoints(pointsRecord);
 ...
 */
export async function addTaskPoints(employeeId, taskId, priority) {
    const pointsRecord = await ensurePointsRecord(employeeId);
    await checkAndResetMonthlyPoints(pointsRecord);
 ...
 */
export async function addProjectCompletionPoints(employeeIds, projectId, isEarlyCompletion = false) {
    const results = [];
 ...
 */
export async function addMilestonePoints(employeeId, milestoneType, description) {
    const pointsRecord = await ensurePointsRecord(employeeId);
    await checkAndResetMonthlyPoints(pointsRecord);
 ...
 * Add penalty points
 */
export async function addPenaltyPoints(employeeId, penaltyAmount, reason) {
    const pointsRecord = await ensurePointsRecord(employeeId);
    await checkAndResetMonthlyPoints(pointsRecord);
 ...
 */
export async function getEmployeePointsSummary(employeeId) {
    try {
...
export async function getPointsLeaderboard(limit = 10) {
    const leaderboard = await Points.find({ isActive: true })
        .sort({ totalPoints: -1 })
        .limit(limit)
 ...
 * Get points history
 */
export async function getPointsHistory(employeeId, limit = 50) {
    const pointsRecord = await ensurePointsRecord(employeeId);
    return pointsRecord.transactions.slice(-limit).reverse();
}
export { POINTS_CONFIG };
//# sourceMappingURL=pointsCalculator.js.map