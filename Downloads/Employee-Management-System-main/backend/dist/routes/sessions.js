...
sessionRoutes.patch('/:id/status', async (req, res) => {
    try {
        ...
        const session = await ProjectSession.findByIdAndUpdate(sessionId, {
            status,
...
sessionRoutes.get('/admin/active', async (req, res) => {
    try {
...
sessionRoutes.get('/admin/summary/:employeeId/:date', async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
