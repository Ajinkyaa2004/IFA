...
taskRoutes.post('/', roleMiddleware(['admin']), async (req, res) => {
    try {
...
taskRoutes.get('/admin/all', roleMiddleware(['admin']), async (req, res) => {
    try {
...
taskRoutes.patch('/:id/status', async (req, res) => {
    try {
        ...
        if (status === 'completed' && oldStatus !== 'completed') {
            pointsResult = await addTaskPoints(task.assignedTo.toString(), task._id.toString(), task.priority);
        }
        res.json({
...
taskRoutes.patch('/:id/progress', async (req, res) => {
    try {
...
taskRoutes.put('/:id', roleMiddleware(['admin']), async (req, res) => {
    try {
        const { title, description, assignedTo, priority, dueDate } = req.body;
        const task = await Task.findByIdAndUpdate(req.params.id, { title, description, assignedTo, priority, dueDate }, { new: true }).populate('assignedBy assignedTo projectId');
        if (!task) {
            return res.status(404).json({ error: 'Task not found' });
        }
        res.json({
...
taskRoutes.delete('/:id', roleMiddleware(['admin']), async (req, res) => {
    try {
