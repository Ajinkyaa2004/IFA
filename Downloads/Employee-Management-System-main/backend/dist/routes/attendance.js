...
attendanceRoutes.get('/today', async (req, res) => {
    try {
        const employeeId = req.user?.id;
        const today = new Date();
        today.setHours(0, 0, 0, 0);
...
attendanceRoutes.get('/date/:date', async (req, res) => {
    try {
        const employeeId = req.user?.id;
...
attendanceRoutes.get('/history', async (req, res) => {
    try {
        const employeeId = req.user?.id;
...
attendanceRoutes.get('/admin/date/:date', async (req, res) => {
    try {
...
attendanceRoutes.get('/admin/range', async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
attendanceRoutes.get('/admin/realtime', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
attendanceRoutes.put('/:id/status', async (req, res) => {
    try {
        ...
        if (['Absent', 'On Leave', 'Half-day'].includes(status)) {
            // End active sessions
            await ProjectSession.updateMany({ attendanceId, sessionEndTime: { $exists: false } }, { sessionEndTime: new Date(), status: 'Offline' });
        }
        await attendance.save();
...
attendanceRoutes.get('/admin/report/weekly', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
        ...
        const weeklyReport = await Promise.all(employees.map(async (employee) => {
            const attendance = await Attendance.find({
            ...
            const stats = {
                ...
                projects: [...new Set(attendance
                        ...
                        .map(a => a.selectedProjectId?.title)
                        .filter(title => title))],
            };
            return {
                employeeId: employee._id,
...
attendanceRoutes.get('/admin/report/monthly', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
        ...
        const employees = await User.find({ role: 'employee' }).select('firstName lastName email');
        const monthlyReport = await Promise.all(employees.map(async (employee) => {
            const attendance = await Attendance.find({
                employeeId: employee._id,
                date: { $gte: startDate, $lte: endDate },
            }).populate('selectedProjectId', 'title');
...
attendanceRoutes.get('/admin/report/employee/weekly/:employeeId', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
attendanceRoutes.get('/admin/report/employee/monthly/:employeeId', authMiddleware, async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
