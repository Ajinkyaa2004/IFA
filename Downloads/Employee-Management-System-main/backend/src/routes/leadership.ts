...
leadershipRoutes.get('/', authMiddleware, roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
...
leadershipRoutes.get('/employee/:employeeId', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const leadership = await Leadership.findOne({ employeeId: req.params.employeeId }).populate(
      'employeeId',
...
leadershipRoutes.post('/', authMiddleware, roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (leadershipDoc) {
      ...
      leadershipDoc.assignedBy = req.user?.id as any;
      await leadershipDoc.save();
    } else {
      leadershipDoc = new Leadership({
        employeeId,
        leadership,
        assignedBy: req.user?.id as any,
      });
      await leadershipDoc.save();
    }
...
leadershipRoutes.delete('/:id', authMiddleware, roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
