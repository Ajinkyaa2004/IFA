...
import { HackathonEvent, HackathonUpdate } from '../models/Hackathon';
...
router.get('/:id/player/:playerId/updates', async (req: Request, res: Response) => {
  try {
...
router.put('/:id/update/:updateId', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const updatedUpdate = await HackathonUpdate.findByIdAndUpdate(
      req.params.updateId,
      {
...
router.put('/:id/player/:playerId/bonus', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (bonusMultiplier) player.bonusMultiplier = bonusMultiplier;
    if (completedInOneDay !== undefined) player.completedInOneDay = completedInOneDay;
...
// Update player score
router.put('/:id/player/:playerId/score', authMiddleware, async (req: Request, res: Response) => {
  try {
...
router.post('/:id/task', authMiddleware, async (req: Request, res: Response) => {
  try {
...
router.get('/:id/player/:playerId/tasks', async (req: Request, res: Response) => {
  try {
    const hackathon = await HackathonEvent.findById(req.params.id);
    if (!hackathon) {
      return res.status(404).json({ message: 'Hackathon not found' });
...
// Delete task
router.delete('/:id/task/:taskId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const hackathon = await HackathonEvent.findById(req.params.id);
    if (!hackathon) {
      return res.status(404).json({ message: 'Hackathon not found' });
