...
automationRoutes.get('/', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
...
automationRoutes.get('/:id', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
...
automationRoutes.post('/', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
...
automationRoutes.put('/:id', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
...
automationRoutes.post('/:id/execute', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const job = await AutomationJob.findById(req.params.id);
    if (!job) {
      return res.status(404).json({ error: 'Job not found' });
...
automationRoutes.delete('/:id', roleMiddleware(['admin']), async (req: AuthRequest, res: Response) => {
  try {
