 ...
 */
router.get('/my-summary', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
 ...
 */
router.get('/my-history', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
    const employeeId = req.user?.id;
    if (!employeeId) {
      return res.status(401).json({ error: 'Unauthorized' });
 ...
 */
router.get('/leaderboard', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
 ...
 */
router.get('/admin/all', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
    ...
    const formatted = allPoints.map((record: any, index: number) => ({
      rank: index + 1,
 ...
 */
router.get('/admin/employee/:employeeId', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
    const userRole = req.user?.role;
    if (userRole !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
    ...
    const pointsRecord: any = await Points.findOne({ employeeId }).populate('employeeId', 'firstName lastName email');
 ...
 */
router.post('/admin/penalty', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
    const userRole = req.user?.role;
    if (userRole !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
 ...
 */
router.get('/admin/summary', authMiddleware, async (req: AuthRequest, res: any) => {
  try {
    const userRole = req.user?.role;
    if (userRole !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
