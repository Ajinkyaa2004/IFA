...
import { WhatsAppLinkModal } from '@/components/applicant/WhatsAppLinkModal';
...
export const GameWrapper: React.FC = () => {
  ...
  const handleGameComplete = useCallback(async (score: number, metadata: number, failed?: boolean, failureReason?: string) => {
    ...
    try {
      ...
      // Show WhatsApp link modal after game completion
      try {
        ...
        if (gameType === 'unblock-me') {
          link = settings.whatsappLinkGame1 || '';
          gameNum = 1;
        } else if (gameType === 'minesweeper') {
          link = settings.whatsappLinkGame2 || '';
          gameNum = 2;
        } else if (gameType === 'water-capacity') {
          link = settings.whatsappLinkGame3 || '';
          gameNum = 3;
        }
        ...
        if (link) {
          ...
          // If no link is set, navigate directly
          navigate('/applicant/assessment');
        }
      } catch (error) {
